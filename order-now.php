			...
			<form action="/wp-json/orders/v1/create" data-ajax  class="form" method="POST">
				...
				<div class="form__box">
					...
					<div class="form__content">
						...
						<div class="form-label-row form-label-row-textarea">
							<label class="form-label">
								<textarea name="add-info" id="add-info" autocomplete="off" maxlength="2000" placeholder="Additional instructions" class="input"></textarea>
							</label>
						</div>
...
<!-- 					<button type="submit" class="button button-fill">Place an order</button>
			...
			<div class="order form__box">
				...
				<button type="button" id="externalSubmitBtn" class="button button-fill" style='margin:30px auto 0'>Place an order</button>
				<p class="form-success-message" style="display:none; color: green; margin-top: 30px;text-align:center;">
					Thank you! Your order has been successfully submitted.
				</p>

				<div class="mobile__buttons" id="payment-buttons" style="display:none;">
					...
					<div class="order__agree">
						...
						<div class="checkbox agree">
							...
							<a href="https://apostil.co/terms-of-use/" target='_blank'>Terms and
					...
					<span style='align-self: flex-start;'>Choose a payment method:</span>
					...
					<p style='margin: 20px auto 0; font-size: 1rem; line-height: 150%;text-align: center;'>* Please note that prices are subject to change depending on each case. Processing Times noted above are estimates only and include the time required for the government agencies issuing the
						Apostille.</p>
				</div>
...
<script>
	document.addEventListener('DOMContentLoaded', function () {
		...
		const zellePay = document.getElementById('zelle-pay');
		if (zellePay) {
			zellePay.addEventListener('click', function () {
				this.classList.toggle('is-flipped');
			});
		}

		const cashPay = document.getElementById('cash-pay');
		if (cashPay) {
...
<!-- 	<script>
		...
		paypal.Buttons({
			...
			createOrder: function(data, actions) {
				...
				let amount = getTotalAmount().toFixed(2);
				if (!amount || amount <= 0) {
