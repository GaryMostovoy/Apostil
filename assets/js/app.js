(() => {
	...
	function tabs() {
		function destroyTabs() {
			const tabs = document.querySelectorAll("[data-tabs]._tab-init");
			tabs.forEach((tabsBlock => {
				tabsBlock.classList.remove("_tab-init");
				tabsBlock.removeEventListener("click", setTabsAction);
				tabsBlock.removeAttribute("data-tabs-index");
				let tabsTitles = tabsBlock.querySelectorAll("[data-tabs-title]");
				let tabsContent = tabsBlock.querySelectorAll("[data-tabs-item]");
				tabsTitles.forEach((title => {
					title.removeAttribute("data-tabs-title");
					title.classList.remove("_tab-active");
				}));
				tabsContent.forEach((content => {
					content.removeAttribute("data-tabs-item");
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function isMobile() {
			return window.matchMedia('(max-width: 61.99875em)').matches;
		}

		// Делегированный обработчик кликов внутри меню
		menuBody.addEventListener('click', function (e) {
			// ищем ближайший LI с классом menu-item-has-children (если нет — ничего не делаем)
			const li = e.target.closest('li.menu-item-has-children');
			if (!li) return;

			// если не мобайл — не мешаем (hover/клики по ссылке работают как обычно)
			if (!isMobile()) return;

			// если клик по ссылке внутри самого подменю — разрешаем навигацию
			const anchor = e.target.closest('a');
			if (anchor && anchor.closest('.sub-menu')) {
				// это ссылка внутри вложенного подменю — пусть работает
				return;
			}

			// предотвращаем переход по родительской ссылке и переключаем подменю
			// (если клик был по ссылке родителя или по самому li)
			if (e.target.closest('a') || e.target === li || li.contains(e.target)) {
				e.preventDefault();
				// переключаем класс
				const isOpen = li.classList.toggle('_submenu-open');
				li.setAttribute('aria-expanded', isOpen ? 'true' : 'false');

				// опционально — закрываем остальные открытые подменю (раскомментируйте, если нужно)
				// closeOtherSubmenus(li);
			}
		});

		// Закрываем подменю при клике вне меню (удобно на мобилке)
		document.addEventListener('click', function (e) {
			if (!isMobile()) return;
			if (!menuBody.contains(e.target)) {
				document.querySelectorAll('.menu nav ul li._submenu-open').forEach(function (el) {
		...
		// При смене размера (уход на десктоп) — убираем мобильные классы/атрибуты
		...
		// (опционально) функция закрытия других подменю, чтобы открыт был только один
	...
	function collectOrderData(form) {
		...
		const documentTypes = Array.from(
			form.querySelectorAll('[name="document-type[]"] option:checked')
		)
			.map(el => el.value)
			.filter(v => v);

		const serviceTypes = Array.from(
			form.querySelectorAll('[name="service-type[]"] option:checked')
		)
			.map(el => el.value)
			.filter(v => v);

		const quantity = parseInt(form.querySelector('[name="quantity"]').value) || 0;
		const translation = parseInt(form.querySelector('[name="translation"]').value) || 0;

		const data = {
			destinationCountry: form.querySelector('[name="country"]').value || '',
			documentTypes,
			quantity,
			translateFrom: form.querySelector('[name="translate-from"]').value || '',
			translateTo: form.querySelector('[name="translate-to"]').value || '',
			translation,
			serviceTypes,
			processingType: form.querySelector('[name="processing-type"]').value || '',
			deliveryMethod: form.querySelector('[name="delivery"]').value || '',
			deliveryAddress: {
				firstName: form.querySelector('[name="delivery-name"]').value || '',
				lastName: form.querySelector('[name="delivery-soname"]').value || '',
				companyName: form.querySelector('[name="delivery-company"]').value || '',
				country: form.querySelector('[name="delivery-country"]').value || '',
				address: form.querySelector('[name="delivery-adress"]').value || '',
				city: form.querySelector('[name="delivery-city"]').value || '',
				state: form.querySelector('[name="delivery-state"]').value || '',
				zipCode: form.querySelector('[name="delivery-zip"]').value || '',
				phoneNumber: form.querySelector('[name="delivery-tel"]').value || '',
				email: form.querySelector('[name="delivery-email"]').value || ''
			},
			returnAddress: {
				firstName: form.querySelector('[name="return-name"]').value || '',
				lastName: form.querySelector('[name="return-soname"]').value || '',
				companyName: form.querySelector('[name="return-company"]').value || '',
				country: form.querySelector('[name="return-country"]').value || '',
				address: form.querySelector('[name="return-adress"]').value || '',
				city: form.querySelector('[name="return-city"]').value || '',
				state: form.querySelector('[name="return-state"]').value || '',
				zipCode: form.querySelector('[name="return-zip"]').value || '',
				phoneNumber: form.querySelector('[name="return-tel"]').value || ''
			},
			additionalInfo: form.querySelector('[name="add-info"]')?.value || '',
			platform,
			device
		};
	...
	function uploadAreaForm() {
		...
		function addFiles(files) {
			for (let file of files) {
				if (!allowedTypes.includes(file.type) || file.size > maxFileSize) {
					...
					uploadMessage.innerHTML = "❌ Error: Unsupported file type or file is too large";
					return;
				}
		...
		function renderFiles() {
			...
			if (selectedFiles.length) {
				...
				uploadMessage.innerHTML = `✅ ${selectedFiles.length} file(s) ready`;
			} else {
				uploadArea.classList.remove("file-loaded", "file-error");
				uploadMessage.innerHTML = "Drag & drop files or click to browse";
			}
		...
		// функція для додавання файлів в FormData
		return function appendFilesToFormData(formData) {
	...
	function formSubmit() {
		...
		if (forms.length) for (const form of forms) {
			form.addEventListener("submit", (function (e) {
				...
				if (total && totalInput) {
					totalInput.value = total.textContent.trim();
				}
		...
		const appendFiles = uploadAreaForm(); // ініціалізація uploadArea
		async function formSubmitAction(form, e) {
			...
			if (error === 0) {
				...
				if (ajax) {
					e.preventDefault();
					// Блокування кнопки на час відправки
					//const submitBtn = form.querySelector('button[type="submit"]');
					const submitBtn = document.getElementById('externalSubmitBtn');
					if (submitBtn) {
						submitBtn.disabled = true;
						submitBtn.classList.add('_sending');
					...
					items.forEach((item, index) => {
						...
						if (["PAID", "FREE"].includes(rawPrice)) return;
					...
					const orderData = collectOrderData(form); // об'єкт з даними формы
					const formData = new FormData();

					// JSON рядок для order_data
					formData.append('order_data', JSON.stringify(orderData));

					formData.append('total', getTotalAmount().toFixed(2));

					// Додаємо файл із input[type=file] 
					...
					if (response.ok) {
						...
						// GTM	
						// Збір даних із всіх .order__data блоків
						const orderDataElements = document.querySelectorAll('.order__data');
						const orderItems = [];

						orderDataElements.forEach(item => {
							const title = item.querySelector('p')?.innerText.trim() || '';
							const priceText = item.querySelector('h3')?.innerText.trim() || '';
							const price = parseFloat(priceText.replace(/[^0-9.]/g, ''));

							if (title && !isNaN(price)) {
								let quantity = 1;

								// Визначення кількості сторінок для перекладу. 1 сторінка - 35$
								if (/translations/i.test(title)) {
									// ділимо ціну на 35
									quantity = Math.round(price / 35);
								}
						...
						const totalText = document.querySelector('#grand')?.innerText.trim() || '';
						const totalValue = parseFloat(totalText.replace(/[^0-9.]/g, '')) || 0;

						const transactionId = document.querySelector('#order-number')?.innerText.trim() || 'unknown';
						...
						if (submitBtn) {
							submitBtn.disabled = true;
							submitBtn.classList.remove('_sending');
		...
		function formSent(form, responseResult = {}) {
			...
			//  Показати номер замовлення
			if (responseResult.orderNumber) {
				const orderNumberElement = document.getElementById("order-number");
				if (orderNumberElement) {
					orderNumberElement.textContent = responseResult.orderNumber;
				}
			}

			//  Показати попап (якщо є))
			if (modules_flsModules.popup) {
				const popup = form.dataset.popupMessage;
				if (popup) modules_flsModules.popup.open(popup);
			}

			formLogging(`Форму відправлено! Номер замовлення: ${responseResult.orderNumber}`);
		}

		document.addEventListener("formSent", async function () {

			try {

				console.log("Payment loader triggered");
				...
				const response = await fetch(
					`/wp-admin/admin-ajax.php?action=get_order_for_payment&order=${encodeURIComponent(orderNumber)}`
				);
				...
				const buttonsBlock = document.getElementById('payment-buttons');
				if (buttonsBlock) {
				...
				// ✅ Wise (оставляем как есть)
				const wise = document.getElementById('wise-pay');
				if (wise && data.total) {
					wise.href =
						`https://wise.com/pay/business/apostilinc?amount=${data.total}&description=Pay%20order%20${orderNumber}`;
				}

				// ✅ Stripe — БЕЗ fetch
				const stripe = document.getElementById('stripe-pay');
				if (stripe && data.stripe_url) {
					stripe.href = data.stripe_url;
				}

				// ✅ PayPal — БЕЗ fetch
				const paypal = document.getElementById('paypall-pay');
				if (paypal && data.paypal_url) {
					paypal.href = data.paypal_url;
				}

				// Visa
				const visa = document.getElementById('visa-pay');
				if (visa && data.paypal_url) {
					visa.href = data.paypal_url;
				}

				console.log("Payment links successfully injected");

				['stripe-pay', 'paypall-pay', 'visa-pay', 'wise-pay'].forEach(id => {
		...
		let externalSubmitBtn = document.getElementById('externalSubmitBtn');
		...
		function waitForOrderNumber() {
			return new Promise(resolve => {
				...
				if (el.textContent.trim()) {
					return resolve(el.textContent.trim());
				}
		...
		function validateBeforePayment() {
			...
			const agree = document.getElementById('c_11')?.checked;
			...
			if (!orderNumber) {
				alert(
					"1. Complete the order information first.\n" +
	...
	class SelectConstructor {
		...
		getSelectTitleValue(selectItem, originalSelect) {
			...
			// додаємо пробіл після коми при мультивиборі
			let selectTitleValue = this.getSelectedOptionsData(originalSelect).elements.map(option => option.textContent.trim()).join(', ');
	...
	// 	ІНІЦІАЛІЗАЦІЯ SELECT ТАК ПОТРІБНО ДЛЯ АРІ
	window.modules_flsModules = window.modules_flsModules || {};

	window.initSelects = function () {
		if (window.modules_flsModules.select) return;
		window.modules_flsModules.select = new SelectConstructor({});
	};
	...
	function copyValueDataForm() {
		...
		if (radioNo) document.addEventListener("DOMContentLoaded", (function () {
			...
			const fieldPairs = [["delivery-name", "return-name"], ["delivery-soname", "return-soname"], ["delivery-adress", "return-adress"], ["delivery-company", "return-company"], ["delivery-city", "return-city"], ["countryFrom", "countryTo"], ["delivery-zip", "return-zip"], ["delivery-tel", "return-tel"], ["usStatesFrom", "usStatesTo"]];
			function copyValues() {
			...
			radioYes.addEventListener("change", (() => {
				if (radioYes.checked) clearValues();
			}));
		}));
	}
	let tabsInstance = null;
	function initTabsWithMediaCheck() {
		const mediaQuery = window.matchMedia("(min-width: 993px)");
		if (mediaQuery.matches) tabsInstance = tabs();
		mediaQuery.addEventListener("change", (e => {
			if (e.matches) tabsInstance = tabs(); else if (tabsInstance && typeof tabsInstance.destroyTabs === "function") tabsInstance.destroyTabs();
		}));
	}
	...
	// Вибір значення chekbox, його передача на сторінку калькулятора в select
	document.addEventListener('DOMContentLoaded', function () {

		const buttons = document.querySelectorAll('.button-fill-order');
		...
		buttons.forEach(button => {
			button.addEventListener('click', (e) => {
				...
				const checkboxes = document.querySelectorAll(
					'.choose-home-page__list .checkbox__input'
				);
				...
				const url = new URL('https://apostil.co/prices/');

				if (selectedValues.length > 0) {
					url.searchParams.set('documents', selectedValues.join(','));
				}
	...
	document.addEventListener("DOMContentLoaded", function () {
		if (window.location.pathname === "/order-now/") {
			const btn = document.querySelector('a.header__desk.button.button-fill[href="https://apostil.co/order-now/"]');
			if (btn) {
				btn.href = "https://wa.me/19297762142";
				btn.textContent = "Call Now";
			}
		}
	});
	document.addEventListener('DOMContentLoaded', function () {
		const popupTrigger = document.querySelectorAll('.hero-about-page__right');
		const popup = document.querySelector('#popup-video');
		const iframe = popup.querySelector('.vimeo-player');
		...
		if (popupTrigger) {
			popupTrigger.forEach(trigger => {
				trigger.addEventListener('click', function () {
					...
					if (videoId) {
						iframe.src = `https://player.vimeo.com/video/${videoId}?autopause=0&title=0&byline=0&portrait=0&autoplay=1`;
					}
				});
			});

			// Очистка src при закритті, щоб відео зупинялося
	...
	// Посилання на store, в залежності, від пристрою
	document.addEventListener("DOMContentLoaded", function () {
		...
		const iosLink = "https://apps.apple.com/us/app/apostil/id6756267706";
		const androidLink = "https://play.google.com/store/apps/details?id=co.apostil";

		let finalLink = iosLink;
